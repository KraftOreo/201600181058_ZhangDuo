...
print(data)
chars = set(data)  # your code#  # 得到输入文件中所有字符种类
data_size, vocab_size = len(data), len(chars)  # your code##统计文件字符数和字符种类数
print('data has %d characters, %d unique.' % (data_size, vocab_size))

char_to_ix = dict(zip(chars, range(len(chars))))  # your code# #构成从字母到数字的映射
...
def lossFun(inputs, targets, hprev):
    ...
    for t in range(len(inputs)):
        ...
        # hs[t] 是t时刻的hidden state， active function = np.tanh(z)，z = Wx*x_t+Wh*hs_(t-1) + bh,
        # 即本时刻输入层+一时刻个隐含层作为Z
        hs[t] = np.tanh(Wxh.dot(xs[t]) + Whh.dot(hs[t - 1]) + bh)  # your code# # hidden state
        # ys[t] = w*hs[t]+by
        ys[t] = Why.dot(hs[t]) + by  # your code# # unnormalized log probabilities for next chars
        # softmax(ys)
        ps[t] = np.exp(ys[t]) / np.sum(np.exp(ys[t]))  # your code# # probabilities for next chars
        # 计算loss = cross_entropy（）
        loss += -np.log(ps[t][targets[t], 0])  # your code# # softmax (cross-entropy loss)
    ...
    for t in reversed(range(len(inputs))):
        # dy是softmax层求导，cross_entropy softmax 求导 aj-yi,yi为one-hot标签,
        # aj为softmax之后第j个神经元输出，详情请见https://blog.csdn.net/u014313009/article/details/51045303
        dy = np.copy(ps[t])  # your code#
        dy[targets[t]] -= 1  # your code# # backprop into y.
        # 反向传播，求Why与by的导数
        dWhy += dy.dot(hs[t].T)  # your code#
        dby += dy  # your code#
        # 反向传播到hidden state请参考https://blog.csdn.net/wjc1182511338/article/details/79191099完成，
        # 其中dh处反向传播的梯度外需加上dhnext
        dh = Why.T.dot(dy) + dhnext  # your code# # backprop into h
        dhraw = (1 - hs[t] * hs[t]) * dh  # your code# # backprop through tanh nonlinearity
        dbh += (1 - hs[t] * hs[t]) * dh  # your code#
        dWxh += dhraw.dot(xs[t].T)  # your code#
        dWhh += dhraw.dot(hs[t - 1].T)  # your code#
        dhnext = Whh.T.dot(dhraw)  # your code#
