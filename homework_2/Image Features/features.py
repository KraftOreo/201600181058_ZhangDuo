...
# # We have seen that we can achieve reasonable performance on an image classification task
# # by training a linear classifier on the pixels of the input image.
# # In this exercise we will show that we can improve our classification performance
# # by training linear classifiers not on raw pixels but on features
# # that are computed from the raw pixels.
...
# train SVMs on top of the features extracted above; this should achieve better results than
...
# # Earlier in this assigment we saw that training a three-layer neural network
# # on raw pixels achieved better classification performance than linear classifiers on raw pixels.
...
# # This approach should outperform all previous approaches: you should easily be able to achieve
# # over 55% classification accuracy on the test set;
#
#
    ...
    for lrdecay in [0.99, 0.95, 0.7, 0.5]:
        for batch_size in [256, 128, 64, 32]:
            for reg in [1, 0.5, 0.25, 0.1]:
                print('lr={}, lrdecay={}, batch_size={}, reg={}'.format(lr, lrdecay, batch_size, reg))
                with open('train_result.txt', 'a+') as f:
                    # for i in range(9):
                    #     mask = list(range(i * int(N / 10), (i + 1) * int(N / 10)))
                    #     if i == 0:
                    #         others = list(range((i + 1) * int(N / 10), N))
                    #     elif i == 9:
                    #         others = list(range(i * int(N / 10)))
                    #     else:
                    #         others = list(range(i * int(N / 10)))
                    #         others1 = list(range((i + 1) * int(N / 10), N))
                    #         others.extend(others1)
                    #     X_val = X[mask]
                    #     X_train = X[others]
                    #     y_val = Y[mask]
                    #     y_train = Y[others]
                    net.train(X_train, y_train, X_val, y_val, learning_rate=lr,
                                  learning_rate_decay=lrdecay, reg=reg, num_iters=3000, batch_size=batch_size,
                                  verbose=True)
