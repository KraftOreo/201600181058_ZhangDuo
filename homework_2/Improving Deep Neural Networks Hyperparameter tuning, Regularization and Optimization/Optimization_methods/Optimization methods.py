...
def random_mini_batches(X, Y, mini_batch_size=64, seed=0):
    ...
    for k in range(0, num_complete_minibatches):
        ### START CODE HERE ### (approx. 2 lines)
        mini_batch_X = shuffled_X[:, k].reshape((X.shape[0], 1))
    ...
    if m % mini_batch_size != 0:
        ### START CODE HERE ### (approx. 2 lines)
        mini_batch_X = shuffled_X[:, -1:-1 - (m % mini_batch_size):-1]
        mini_batch_Y = shuffled_Y[:, -1:-1 - (m % mini_batch_size):-1]
        ### END CODE HERE ###
        mini_batch = (mini_batch_X, mini_batch_Y)
        mini_batches.append(mini_batch)
...
def update_parameters_with_adam(parameters, grads, v, s, t, learning_rate=0.01,
    ...
    for l in range(L):
        ...
        parameters["W" + str(l + 1)] -= learning_rate * (
                ...
                np.sqrt(s_corrected["dW" + str(l + 1)]) + epsilon * np.ones_like(s_corrected["dW" + str(l + 1)])))
        parameters["b" + str(l + 1)] -= learning_rate * (
                v_corrected["db" + str(l + 1)] / (
                np.sqrt(s_corrected["db" + str(l + 1)]) + epsilon * np.ones_like(s_corrected["db" + str(l + 1)])))
        ### END CODE HERE ###
